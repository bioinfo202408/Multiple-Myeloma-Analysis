...
# ========== Data loading ==========
train_data <- vroom(
   "/home/yjliu/mmProj/data_process/Human/Ensemble_Model/Stacking/predictions/10model_train_predictions.csv", 
   delim = ",", 
)
...
test_data <- vroom(
   "/home/yjliu/mmProj/data_process/Human/Ensemble_Model/Stacking/predictions/10model_test_predictions.csv", 
   delim = ",", 
)
...
external_data1 <- vroom(
   "/home/yjliu/mmProj/data_process/Human/Ensemble_Model/Stacking/predictions/10model_external1_predictions.csv", 
   delim = ",", 
)
...
external_data2 <- vroom(
   "/home/yjliu/mmProj/data_process/Human/Ensemble_Model/Stacking/predictions/10model_external2_predictions.csv", 
   delim = ",", 
)
...
# ========== 2. Use caret for 10-fold cross-validation, tune regularization parameters ==========
...
output_dir <- "/home/yjliu/mmProj/data_process/Human/Ensemble_Model/Stacking/predictions/ridge/"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

pdf(paste0(output_dir, "ridge_regression_ROC_all.pdf"), width = 6, height = 6)
plot(roc_test, col = "darkgreen", lwd = 2, main = "ridge_regression ROC: Test vs External1 vs External2")
...
write.csv(importance_df, paste0(output_dir, "ridge_regression_feature_importance_full.csv"), row.names = FALSE)
cat("✅ Feature importance saved\n")

# ========== 9. Plot coefficient figure ==========
pdf(paste0(output_dir, "ridge_regression_coefficient_plot.pdf"), width = 10, height = 6)
...
ggplot(top_features, aes(x = reorder(Feature, Odds_Ratio), y = Odds_Ratio)) +
   ...
   labs(x = "Feature", y = "Odds Ratio", 
        title = "Top Feature Odds Ratios (ridge_regression Regression)",
...
pdf(paste0(output_dir, "ridge_regression_Probability_distribution_all.pdf"), width = 12, height = 4)
par(mfrow = c(1, 3))
...
print("Confusion matrix for external validation set 2:"); print(cm_external2)
...
write.table(metrics_combined,
            file = paste0(output_dir, "ridge_regression_Performance_all.txt"),
...
pdf(paste0(output_dir, "ridge_regression_Model_metrics_barplot_all.pdf"), width = 10, height = 5)
ggplot(metric_df_long, aes(x = Metric, y = Value, fill = Dataset)) +
   ...
   ylim(0, 1) +
   labs(y = "Score", title = "ridge_regression Regression Model Performance Metrics", fill = "Dataset") +
   theme_bw() +
...
# Save PR curves
pdf(paste0(output_dir, "ridge_regression_PR_curve_all.pdf"), width = 6, height = 6)
plot(pr_test, color = "darkgreen", lwd = 2, main = "ridge_regression Regression PR Curve: Test vs External1 vs External2")
...
saveRDS(logit_model_final, file.path(model_package_dir, "ridge_regression_model.rds"))
...
predict_ridge_regression_model <- function(new_data, model_dir = model_package_dir, threshold = NULL) {
   # Load dependency information
   preproc <- readRDS(file.path(model_dir, "preprocessing_info.rds"))
   model <- readRDS(file.path(model_dir, "ridge_regression_model.rds"))
...
saveRDS(predict_ridge_regression_model, file.path(model_package_dir, "predict_function.rds"))

cat("✅ Model package saved successfully! Path:", model_package_dir, "\n")
cat("✅ Ridge regression modeling workflow fully completed\n")