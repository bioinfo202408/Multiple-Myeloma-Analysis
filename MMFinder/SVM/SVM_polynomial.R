...
library(kernlab)    # Kernel function support
library(dplyr)
library(vroom)
library(ggplot2)
library(tidyr)
...
# Define tuning grid, polynomial-kernel SVM hyperparameters
tune_grid <- expand.grid(
   degree = c(2, 3, 4),      # Polynomial degree
   scale = c(0.001, 0.01, 0.1, 1),  # Scale parameter, gamma
   C = c(0.1, 1, 10, 100)    # Penalty parameter
)
...
# Cross-validation tuning, using svmPoly, polynomial kernel
cat("Start 10-fold cross-validation tuning...\n")
cv_model <- train(
...
svm_model_full <- svm(
   ...
   probability = TRUE,
   cost = 1,
   gamma = 0.001,  # The scale in the polynomial kernel corresponds to the gamma parameter
   degree = 4,
   coef0 = 1,  # Constant term of the polynomial kernel, usually set to 0 or 1
   scale = FALSE  # Do not perform internal standardization
)
save.image(file = "/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM/svm_polynomial.RData")
#load("/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM/svm_polynomial.RData")

# ========== 3. Validate the model on the test set ==========
pred_prob_test <- attr(predict(svm_model_full, test_data, probability = TRUE), 
                       "probabilities")[, "tumor"]
...
pred_prob_external1 <- attr(predict(svm_model_full, external_data1, probability = TRUE), 
                            "probabilities")[, "tumor"]
...
pred_prob_external2 <- attr(predict(svm_model_full, external_data2, probability = TRUE), 
                            "probabilities")[, "tumor"]
...
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/svm_ROC_all.pdf", width = 6, height = 6)
plot(roc_test, col = "darkgreen", lwd = 2, main = "svm ROC: Test vs External1 vs External2")
...
# ========== 7. Extract and save model summary ==========
svm_summary <- summary(svm_model_full)
write.csv(data.frame(
   Model_Type = "svm-RBF",
   Cost = svm_summary$cost,
   Gamma = svm_summary$gamma,
   Number_of_Support_Vectors = svm_summary$nSV[1] + svm_summary$nSV[2],
   Total_Samples = svm_summary$tot.nSV
), "/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/svm_model_summary.csv", row.names = FALSE)
cat("✅ Model summary saved\n")

# ========== 8. Plot probability distribution histograms ==========
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/Probability_distribution_all.pdf", width = 12, height = 4)
par(mfrow = c(1, 3))
...
write.table(metrics_combined,
            file = "/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/Performance_all.txt",
...
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/svm_Model_metrics_barplot_all.pdf", width = 10, height = 5)
ggplot(metric_df_long, aes(x = Metric, y = Value, fill = Dataset)) +
   ...
   ylim(0, 1) +
   labs(y = "Score", title = "svm Model Performance Metrics", fill = "Dataset") +
   theme_bw() +
...
# Save PR curves
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/svm_PR_curve_all.pdf", width = 6, height = 6)
plot(pr_test, color = "darkgreen", lwd = 2, main = "svm PR Curve: Test vs External1 vs External2")
...
model_package_dir <- "/home/yjliu/mmProj/data_process/Human/Machine_Learning/SVM_polynomial/model_package/"
dir.create(model_package_dir, recursive = TRUE, showWarnings = FALSE)

# Save svm model
saveRDS(svm_model_full, file.path(model_package_dir, "svm_model.rds"))
...
model_metadata <- list(
   ...
   package_versions = list(
      e1071 = packageVersion("e1071"),
      kernlab = packageVersion("kernlab"),
...
predict_svm_model <- function(new_data, model_dir = model_package_dir, threshold = NULL) {
   # Load dependency information
   preproc <- readRDS(file.path(model_dir, "preprocessing_info.rds"))
   model <- readRDS(file.path(model_dir, "svm_model.rds"))
   
   # Ensure feature columns are consistent
   new_x <- new_data[, preproc$feature_cols, drop = FALSE]
   
   # Predict probabilities
   pred <- predict(model, new_x, probability = TRUE)
   prob_tumor <- attr(pred, "probabilities")[, "tumor"]
   
   # Determine threshold
   pred_threshold <- ifelse(is.null(threshold), preproc$optimal_threshold, threshold)
   
   # Predict classes
   pred_class <- ifelse(prob_tumor >= pred_threshold, "tumor", "health")
...
saveRDS(predict_svm_model, file.path(model_package_dir, "predict_function.rds"))