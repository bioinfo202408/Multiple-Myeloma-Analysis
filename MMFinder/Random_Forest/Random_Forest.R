...
pred_prob_test <- predict(rf_model_full, test_data)$predictions[, "tumor"]
...
pred_prob_external1 <- predict(rf_model_full, external_data1)$predictions[, "tumor"]
...
pred_prob_external2 <- predict(rf_model_full, external_data2)$predictions[, "tumor"]
...
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/RF_ROC_all.pdf", width = 6, height = 6)
plot(roc_test, col = "darkgreen", lwd = 2, main = "RandomForest ROC: Test vs External1 vs External2")
...
# ========== 7. Extract and save feature importance ==========
importance_df <- data.frame(
   Feature = names(rf_model_full$variable.importance),
   Importance = as.numeric(rf_model_full$variable.importance)
) %>% arrange(desc(Importance))

write.csv(importance_df, "/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/RF_feature_importance_full.csv", row.names = FALSE)
cat("✅ Feature importance saved\n")

# ========== 8. Plot probability distribution histograms ==========
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/Probability_distribution_all.pdf", width = 12, height = 4)
par(mfrow = c(1, 3))
...
write.table(metrics_combined,
            file = "/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/Performance_all.txt",
...
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/RF_Model_metrics_barplot_all.pdf", width = 10, height = 5)
ggplot(metric_df_long, aes(x = Metric, y = Value, fill = Dataset)) +
   ...
   ylim(0, 1) +
   labs(y = "Score", title = "RandomForest Model Performance Metrics", fill = "Dataset") +
   theme_bw() +
...
# Save PR curves
pdf("/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/RF_PR_curve_all.pdf", width = 6, height = 6)
plot(pr_test, color = "darkgreen", lwd = 2, main = "RandomForest PR Curve: Test vs External1 vs External2")
...
model_package_dir <- "/home/yjliu/mmProj/data_process/Human/Machine_Learning/RandomForest/model_package/"
dir.create(model_package_dir, recursive = TRUE, showWarnings = FALSE)

# Save RandomForest model
saveRDS(rf_model_full, file.path(model_package_dir, "randomforest_model.rds"))
...
model_metadata <- list(
   ...
   model_params = list(
      num.trees = rf_model_full$num.trees,
      mtry = rf_model_full$mtry,
      min.node.size = rf_model_full$min.node.size,
      splitrule = rf_model_full$splitrule
   ),
   performance = list(
...
predict_rf_model <- function(new_data, model_dir = model_package_dir, threshold = NULL) {
   # Load dependency information
   preproc <- readRDS(file.path(model_dir, "preprocessing_info.rds"))
   model <- readRDS(file.path(model_dir, "randomforest_model.rds"))
   ...
   prob_tumor <- pred$predictions[, "tumor"]
...
saveRDS(predict_rf_model, file.path(model_package_dir, "predict_function.rds"))
