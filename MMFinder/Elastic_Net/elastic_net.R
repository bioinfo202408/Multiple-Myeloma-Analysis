...
# Logistic regression requires tumor to be the positive class
...
# ========== 2. Perform 10-fold cross-validation using caret to tune regularization parameters ==========
cat("\nStarting 10-fold cross-validation for parameter tuning...\n")
...
# Define tuning grid for regularization parameters
tune_grid <- expand.grid(
   alpha = 0.5,  # 0: Ridge, 0.5: Elastic Net, 1: Lasso
...
# Use glmnet for cross-validation-based tuning
cv_model <- train(
   x = X_train,
   y = y_train,
   method = "glmnet",
   ...
   tuneGrid = tune_grid,
   metric = "ROC",
   preProc = NULL  # No preprocessing is performed because the data have already been standardized
)
...
# ========== 3. Train the final model using the optimal parameters ==========
cat("Training the final logistic regression model...\n")
...
logit_model_final <- glmnet(
   ...
   alpha = 0.25,
   lambda = 0.001,
   standardize = FALSE  # No standardization is performed because the data have already been standardized
)
...
output_dir <- "/home/yjliu/mmProj/data_process/Human/Machine_Learning/elastic_net/"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

pdf(paste0(output_dir, "elastic_net_ROC_all.pdf"), width = 6, height = 6)
plot(roc_test, col = "darkgreen", lwd = 2, main = "elastic_net Regression ROC: Test vs External1 vs External2")
...
write.csv(importance_df, paste0(output_dir, "elastic_net_feature_importance_full.csv"), row.names = FALSE)
cat("Feature importance has been saved.\n")

# ========== 9. Plot coefficient values ==========
pdf(paste0(output_dir, "elastic_net_coefficient_plot.pdf"), width = 10, height = 6)
...
ggplot(top_features, aes(x = reorder(Feature, Odds_Ratio), y = Odds_Ratio)) +
   ...
   labs(x = "Feature", y = "Odds Ratio", 
        title = "Top Feature Odds Ratios (elastic_net Regression)",
...
pdf(paste0(output_dir, "elastic_net_Probability_distribution_all.pdf"), width = 12, height = 4)
par(mfrow = c(1, 3))
...
# ========== 12. Calculate confusion matrices ==========
# Test set
...
print("Test set confusion matrix:"); print(cm_test$table)
...
print("External validation set 1 confusion matrix:"); print(cm_external1)
...
print("External validation set 2 confusion matrix:"); print(cm_external2$table)

# ========== 13. Function to calculate MCC ==========
...
write.table(metrics_combined,
            file = paste0(output_dir, "elastic_net_Performance_all.txt"),
            sep = "\t", quote = FALSE, row.names = FALSE)

# ========== 15. Plot performance metric barplot ==========
metric_df_long <- pivot_longer(metrics_combined, 
                               cols = c(Accuracy, Sensitivity, Specificity, F1_Score, MCC),
                               names_to = "Metric", values_to = "Value")

pdf(paste0(output_dir, "elastic_net_Model_metrics_barplot_all.pdf"), width = 10, height = 5)
ggplot(metric_df_long, aes(x = Metric, y = Value, fill = Dataset)) +
   ...
   ylim(0, 1) +
   labs(y = "Score", title = "elastic_net Regression Model Performance Metrics", fill = "Dataset") +
   theme_bw() +
   theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5))
dev.off()

# ========== 16. Plot PR curves ==========
# Convert labels to numeric values, where health = 0 and tumor = 1
...
# Save PR curves
pdf(paste0(output_dir, "elastic_net_PR_curve_all.pdf"), width = 6, height = 6)
plot(pr_test, color = "darkgreen", lwd = 2, main = "elastic_net Regression PR Curve: Test vs External1 vs External2")
...
saveRDS(logit_model_final, file.path(model_package_dir, "elastic_net_model.rds"))
...
# ========== 18. Save a general prediction function ==========
predict_elastic_net_model <- function(new_data, model_dir = model_package_dir, threshold = NULL) {
   # Load dependency information
   preproc <- readRDS(file.path(model_dir, "preprocessing_info.rds"))
   model <- readRDS(file.path(model_dir, "elastic_net_model.rds"))
   
   # Ensure feature-column consistency
...
saveRDS(predict_elastic_net_model, file.path(model_package_dir, "predict_function.rds"))

cat("Model package has been saved. Path:", model_package_dir, "\n")
cat("Logistic regression modeling workflow has been completed.\n")